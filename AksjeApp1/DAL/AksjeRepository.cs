...
namespace AksjeApp1.DAL
{
    public class AksjeRepository : AksjeRepositoryInterface
    {
        ...
        /*
        public async Task<bool> KjopAtTime(int id, Portfolios innPortfolio)
        {

            int[] aksjePriser = new int[] { 151, 152, 153, 154, 156, 157, 157, 158, 159, 160 };
            int rnd = new Random().Next(11);
            int aksjePris = aksjePriser[rnd];
            if (innPortfolio.Aksje.Pris == aksjePris)
            {
                try
                {
                    ...
                    if (enBruker.Saldo >= innPortfolio.Aksje.Pris * innPortfolio.Antall && enAksje.AntallLedige >= innPortfolio.Antall)
                    {
